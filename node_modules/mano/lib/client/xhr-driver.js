...
send = function (method, url, data) {
	...
	xhr.onload = function () {
		...
		if ((xhr.status < 200) || (xhr.status >= 300)) {
			if (result.message) {
				error = customError(result.message, result);
			} else {
				error = customError("Rejecteded XHR request to " + JSON.stringify(url) + ": " +
					String(result), 'XHR_REQUEST_REJECTED', { originalResult: result });
			}
			def.reject(error);
			return;
		}
	...
	xhr.onerror = function (error) {
		var errorMessage = "Errored XHR requested to " + JSON.stringify(url) + ": ";
		errorMessage += (error ? (error.stack || error.message) : "(no error object)");
		def.reject(customError(errorMessage, 'XHR_REQUEST_ERRORED', { originalError: error }));
	};
	xhr.onabort = function (error) {
		console.warn("XHR Operation aborted (Ok, if request aborted on purpose)");
		var errorMessage = "Aborted XHR requested to " + JSON.stringify(url) + ": ";
		errorMessage += (error ? (error.stack || error.message) : "(no error object)");
		def.reject(customError(errorMessage, 'XHR_REQUEST_ABORTED', { originalError: error }));
	};
