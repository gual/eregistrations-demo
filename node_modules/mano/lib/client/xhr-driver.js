...
send = function (method, url, data) {
	...
	xhr.onerror = function (error) {
		def.reject(customError("Errored XHR requested to " + JSON.stringify(url) + ": " +
			error ? (error.stack || error.message) : "(no error object)", 'XHR_REQUEST_ERRORED',
			{ originalError: error }));
	};
	xhr.onabort = function (error) {
		console.warn("XHR Operation aborted (Ok, if request aborted on purpose)");
		def.reject(customError("Aborted XHR requested to " + JSON.stringify(url) + ": " +
			error ? (error.stack || error.message) : "(no error object)", 'XHR_REQUEST_ABORTED',
			{ originalError: error }));
	};
