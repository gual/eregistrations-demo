...
exports['log-client-error'] = {
	validate: function (data) {
		...
		['location', 'message', 'source', 'line', 'column',
			'errorMessage', 'errorStack'].forEach(function (key) {
			if (parsedData[key]) {
				normalizedData[key] = parsedData[key];
			}
		});
	...
	submit: function (normalizedData) {
		var now = new Date(), content = [], logFileName, messages, data;
		data = normalizeOptions({ clientWithSessionId: this.req.$clientWithSessionId,
			agent: this.req.headers['user-agent'], referer: this.req.headers.referer },
			normalizedData);
		...
		Object.keys(data).forEach(function (key) {
			content.push(messages[key], data[key]);
		});
		content.push('\n\n\n');
		logFileName = [now.getFullYear(), now.getMonth(), now.getDate()];
		logFileName = logFileName.join('-') + '_' + [now.getHours(), now.getMinutes(),
			now.getSeconds(), now.getMilliseconds()].join('-');
